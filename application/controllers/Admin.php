...
class Admin extends MY_Controller
{
	...
	public function groupGameList()
	{
		...
		$dataList = $this->getDataRow($tableName, $select, '', '', $join);
		$data['html']['title'] = 'List Game Group';
		$data['html']['dataList'] = $dataList;
		$data['html']['tableName'] = $tableName;
		$data['html']['form'] = get_class($this) . '/groupGame';
		$data['url'] = 'admin/groupGameList';
		$this->template($data);
	}

	public function groupGame($id = '')
	{
		$tableName = 'game_group';
		$tableDetail = '';
		$baseUrl = get_class($this) . '/' . __FUNCTION__;
		...
		$data['html']['baseUrl'] = $baseUrl;
		$data['html']['title'] = 'Input Data Group Game';
		$data['html']['err'] = $this->genrateErr();
		$data['url'] = 'admin/' . __FUNCTION__ . 'Form';
		$this->template($data);
	}
	public function levelList()
	{
		...
		$dataList = $this->getDataRow($tableName, $select, '', '', $join);
		// memilihan warna
		foreach ($dataList as $dataListKey => $dataListValue) {
			$dataList[$dataListKey]['startcolor'] = $this->colorClass((int)$dataListValue['start']);
			$dataList[$dataListKey]['endcolor'] = $this->colorClass((int)$dataListValue['end']);
		}
	...
	public function level($id = '')
	{
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			if (empty($_POST['start']))
				array_push($arrMsgErr, 'Start Wajib Di isi');
			if (empty($_POST['end']))
				array_push($arrMsgErr, 'End Wajib Di isi');
			if ($_POST['end'] < $_POST['start'])
				array_push($arrMsgErr, 'Start tidak Boleh lebih besar dari End');
		...
		$data['html']['baseUrl'] = $baseUrl;
		$data['html']['title'] = 'Input Data Level';
		$data['html']['err'] = $this->genrateErr();
		$data['url'] = 'admin/' . __FUNCTION__ . 'Form';
		$this->template($data);
	}
	...
	public function gameList()
	{
		...
		$dataList = $this->getDataRow($tableName, $select, '', '', $join);
		foreach ($dataList as $dataListKey => $dataListValue) {
			$dataList[$dataListKey]['percentagecolor'] = $this->colorClass((int)$dataListValue['percentage']);
		}
	...
	public function game($id = '')
	{
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			$level = $this->getDataRow('level', '*', array('pkey' => $_POST['levelKey']))[0];
			$_POST['percentage'] = rand($level['start'], $level['end']);
		...
		$groub = $this->getDataRow('game_group', '*');
		$level = $this->getDataRow('level', '*');

		$data['html']['baseUrl'] = $baseUrl;
		$data['html']['level'] = $level;
		$data['html']['groub'] = $groub;
